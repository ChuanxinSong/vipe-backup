...
def load_raw_slam_sequence(raw_slam_dir: Path, sequence_name: str):
    distance_path = raw_slam_dir / f"{sequence_name}_distance.npy"
    valid_mask_path = raw_slam_dir / f"{sequence_name}_distance_valid_mask.npy"
    pose_path = raw_slam_dir / f"{sequence_name}_poses.json"
    video_path = raw_slam_dir / f"{sequence_name}_vis.mp4"

    missing = [str(path) for path in [distance_path, valid_mask_path, pose_path, video_path] if not path.exists()]
    if missing:
        raise FileNotFoundError(f"Missing raw_slam files for {sequence_name}: {missing}")
    ...
    if valid_masks.shape != depths.shape:
        raise ValueError(
            f"{valid_mask_path} must have the same shape as raw_slam depths. "
    ...
    if frame_shape != depths.shape[1:]:
        raise ValueError(
            f"raw_slam video frame size for {sequence_name} does not match raw_slam depth size: "
            f"{frame_shape} vs {depths.shape[1:]}"
        )

    lengths = {
        "vipe_depth": None,
        "raw_depth": len(depths),
        "raw_valid_mask": len(valid_masks),
        "raw_pose": len(poses),
        "raw_video": len(frames),
    }
    expected_len = lengths["raw_depth"]
    mismatched = {key: value for key, value in lengths.items() if value is not None and value != expected_len}
    if mismatched:
        raise ValueError(f"Length mismatch in raw_slam data for {sequence_name}: {lengths}")

    return depths, valid_masks, poses, frames


def align_vipe_depth_to_raw_slam(
    sequence_name: str,
    vipe_depths: np.ndarray,
    raw_depths: np.ndarray,
    raw_valid_masks: np.ndarray,
    ...
    if len(vipe_depths) != len(raw_depths):
        raise ValueError(
            f"Length mismatch for {sequence_name}: "
            f"ViPE depth has {len(vipe_depths)} frames, raw_slam depth has {len(raw_depths)} frames."
        )

    vipe_resized = resize_depth_sequence(vipe_depths, raw_depths.shape[1:])
    clipped_original = np.clip(vipe_resized, 0.0, 1000.0).astype(np.float32, copy=False)

    x_list = []
    y_list = []

    for frame_idx in range(len(raw_depths)):
        vipe_depth = vipe_resized[frame_idx]
        raw_depth = raw_depths[frame_idx]
        raw_valid = raw_valid_masks[frame_idx] > 0

        mask = (
            raw_valid
            & np.isfinite(raw_depth)
            & (raw_depth > ALIGN_MIN_DISTANCE)
            & (raw_depth < ALIGN_MAX_DISTANCE)
        ...
        if np.any(mask):
            ...
            y_list.append(raw_depth[mask])

    if not x_list:
        print(f"Warning: No valid raw_slam alignment points found for {sequence_name}, keeping original depths.")
    ...
    try:
        ...
        print(f"Warning: raw_slam alignment failed for {sequence_name}, keeping original depths.")
        return clipped_original, None, None, False

    if not np.isfinite(scale) or not np.isfinite(shift):
        print(
            f"Warning: raw_slam alignment produced non-finite parameters for {sequence_name}, "
...
def process_sequence(
    ...
    raw_depths, raw_valid_masks, raw_poses, raw_frames = load_raw_slam_sequence(raw_slam_dir, sequence_name)
    ...
    aligned_depths, align_scale, align_shift, did_align = align_vipe_depth_to_raw_slam(
        sequence_name,
        vipe_depths,
        raw_depths,
        raw_valid_masks,
    ...
    if did_align:
        print(f"{sequence_name}: aligned ViPE depth to raw_slam with scale={align_scale:.6f}, shift={align_shift:.6f}")
    else:
        print(f"{sequence_name}: raw_slam alignment failed, using clipped ViPE depth without alignment.")
    ...
    for anchor_idx in anchors:
        ...
        img0 = (
            torch.from_numpy(raw_frames[anchor_idx].copy()).permute(2, 0, 1).float().unsqueeze(0).to(device) / 255.0
        )
        depth0 = torch.from_numpy(aligned_depths[anchor_idx].copy()).unsqueeze(0).unsqueeze(0).to(device)
        pose0 = torch.from_numpy(raw_poses[anchor_idx].copy()).float().to(device)
        ...
        for offset in offsets:
            ...
            img1_np = raw_frames[target_idx]
            pose1 = torch.from_numpy(raw_poses[target_idx].copy()).float().to(device)
...
def collect_sequence_names(vipe_dir: Path, raw_slam_dir: Path) -> List[str]:
    vipe_sequences = {path.stem.replace("_distance", "") for path in vipe_dir.glob("*_distance.npy")}
    raw_slam_sequences = {path.stem.replace("_distance", "") for path in raw_slam_dir.glob("*_distance.npy")}

    if vipe_sequences != raw_slam_sequences:
        only_vipe = sorted(vipe_sequences - raw_slam_sequences)
        only_raw = sorted(raw_slam_sequences - vipe_sequences)
        raise ValueError(
            "ViPE and raw_slam sequence sets must match exactly. "
            f"Only in ViPE: {only_vipe[:10]}; only in raw_slam: {only_raw[:10]}"
        )

    if not vipe_sequences:
        raise ValueError(f"No sequences found in {vipe_dir}")

    return sorted(vipe_sequences)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate forward consistency using ViPE depth aligned to raw_slam distance and raw_slam poses."
    )
    parser.add_argument(
        "--base_dir",
        type=str,
    ...
    parser.add_argument(
        "--raw_slam_dir",
        type=str,
        default="carla_benchmark_results/genex_results_raw_slam/genex_1024_raw_slam",
        help="Directory containing raw_slam *_distance.npy, *_distance_valid_mask.npy, *_poses.json, *_vis.mp4.",
    )
    parser.add_argument(
        "--output_root",
        type=str,
...
def main():
    ...
    raw_slam_dir = Path(args.raw_slam_dir).expanduser().resolve()

    if not base_dir.exists():
        raise FileNotFoundError(f"ViPE base directory {base_dir} does not exist.")
    if not raw_slam_dir.exists():
        raise FileNotFoundError(f"raw_slam directory {raw_slam_dir} does not exist.")

    output_root = (
        Path(args.output_root).expanduser().resolve()
        if args.output_root is not None
        else base_dir / "forward_consistency_results_raw_slam_align"
    )
    ...
    sequence_names = collect_sequence_names(base_dir, raw_slam_dir)
    print(f"Found {len(sequence_names)} sequences with matching names.")
