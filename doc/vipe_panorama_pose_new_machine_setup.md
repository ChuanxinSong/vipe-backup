# ViPE 全景位姿与 OmniRoam 指标：新机器部署手册
...
为了让不同机器尽量使用相同代码，首先记录并固定 Git commit。编写本文时使用的版本是：

```text
535b55113af6a2922253ef8932245eefe6f0738a
```

该 commit 位于 `vipe-backup` 的 `dev_panorama` 分支，不在当前上游 `nv-tlabs/vipe` 的远端分支中。在新机器上克隆对应仓库，然后进入仓库根目录：
...
cd /path/to/vipe
git checkout 535b55113af6a2922253ef8932245eefe6f0738a
```
...
2. NVIDIA 驱动能够运行 CUDA 12.8 构建的 PyTorch。
...
## 4. 创建 `vipe-panorama` 环境

仓库提供的 `envs/base.yml` 包含 Python 3.10 和 CUDA 12.8 编译依赖。创建环境：
...
PyTorch 的 `cu128` wheel 包含 CUDA 运行库，但不保证包含 CUDA 编译器。ViPE 首次安装需要用 `nvcc` 编译自定义扩展。
...
在 `vipe-panorama` 环境内安装仓库所需的 CUDA 12.8 编译组件：
...
```text
.../envs/vipe-panorama/bin/nvcc
Cuda compilation tools, release 12.8
```

不建议直接安装系统仓库中的 `nvidia-cuda-toolkit`，因为它可能提供与 `torch==2.7.0+cu128` 不一致的 CUDA 版本。
...
确认编译和运行使用的是同一个 `vipe-panorama` 环境。若仓库是从其他机器直接复制过来的，旧 `.so` 可能与新机器不兼容；最稳妥的做法是在干净的仓库 checkout 中重新执行本节的安装命令。
...
仓库的 `envs/requirements.txt` 没有安装 CVXPY，因此按照本文创建的干净环境会使用：

```text
worldscore_scale_solver = scipy_minimize_scalar
```

当前已有的 `0007_840137` 结果也是使用 SciPy 生成的。开发机器现有的 `vipe-panorama` 环境后来额外安装了 `cvxpy==1.7.5`，现在重新评测会改用 CVXPY。两者数值差异通常极小，但严格跨机器复现时必须统一。

建议新机器遵循以下规则：

1. 只安装 `envs/requirements.txt` 中锁定的依赖。
2. 不额外安装 CVXPY。
3. 在每个 summary 中检查：
