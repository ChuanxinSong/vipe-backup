...
def main():
    ...
    video_name = "076_2_segment_09" # You can change this to other sequences 076_2_segment_09
    ...
    for step in eval_steps:
        ...
        img0_warped = F.grid_sample(img0_torch, grid_backward, mode='bilinear', 
                                     padding_mode='zeros', align_corners=True)
        
        # 5. Compute valid mask: grid within bounds and valid depth
        ...
        mask = valid_grid & valid_depth.squeeze(1)
        ...
        vis_path = f"eval_{video_name}_step{step}.png"
        
        # Create warped image with holes highlighted
        img0_warped_with_holes = img0_warped_np.copy()
        # Mark invalid regions (holes) in bright magenta
        img0_warped_with_holes[mask_np < 0.5] = [255, 0, 255]  # Magenta for holes
        ...
        cv2.putText(img0_warped_labeled, f"Frame {idx0} projected to Frame {idx1} (step={step})", (10, 30), 
